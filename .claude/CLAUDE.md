...
- **`AudioEngine`** — Core audio engine implementing `oboe::AudioStreamDataCallback`. Owns one `ClickGenerator` and one `FileDecoder`. Methods:
  - `loadFile(fd, offset, length)` — opens an audio file via `FileDecoder::open` (invoked by `PlaybackViewModel.selectFile`). Resets `mPauseFrameOffset` to 0.
  - `start()` — opens an Oboe output stream (`PerformanceMode::LowLatency`, `SharingMode::Exclusive`, Float stereo, 48 kHz), starts the decode thread, and configures the click generator with the beat-grid offset. Sets `mIsPlaying` atomic to true.
  ...
  - `resume()` — reopens the Oboe stream, calls `fileDecoder.resume(mPauseFrameOffset)` to seek to saved position, reconfigures click generator. Sets `mIsPlaying` to true.
  - `seekToStart()` — sets `mPauseFrameOffset = 0`, calls `stop()`, then `fileDecoder.seekToStart()`. (Does not call `start()`; the Kotlin caller's `play()` does.)
  - `seekTo(positionMs)` — stop-if-playing, set `mPauseFrameOffset` from ms, `fileDecoder.resume(offset)`, restart-if-was-playing.
  - `getDurationMs()` — returns total file duration in milliseconds via `FileDecoder::getDurationFrames()`.
  - `getPositionMs()` — returns `(mPauseFrameOffset + mFileDecoder.getFramesConsumed()) * 1000 / kDefaultSampleRate`.
  - `setFirstBeatOffset(frames)` — stores `mFirstBeatOffset` (atomic int64_t; written from main thread, read from audio callback thread). Used by `computeFramesUntilBeat()`.
  - `isPlaying()` — returns `mIsPlaying.load()` (atomic bool).
  - `onAudioReady` (audio callback) — reads decoded file frames + generates click frames, scales by volume, mixes, and writes to output. Reconfigures click generator on BPM/beatsPerBar change.
  - Setters (`setBpm`, `setBeatsPerBar`, `setTrackVolume`, `setClickVolume`, `setFirstBeatOffset`) use `std::atomic` for lock-free communication from Kotlin/UI thread to audio thread.
  - Private `computeFramesUntilBeat(currentFrameOffset, streamRate)` — computes the delay (in stream-rate frames) until the next beat boundary given `mFirstBeatOffset`. All frame offsets are in 48 kHz units; result is rescaled to `streamRate` for ClickGenerator.

- **`ClickGenerator`** — Synthesizes a metronome click track as float32 sine bursts with exponential decay envelope. Beat 1 of each bar is 880 Hz; beats 2–N are 660 Hz. All clicks are 30 ms long. State: beat interval (in frames), current beat index, click envelope remaining, oscillator phase. `configure(bpm, beatsPerBar, sampleRate, initialOffsetFrames=0)` sets `mFramesUntilNextBeat = max(0, initialOffsetFrames)` so the first beat can be delayed to align with the recording's beat grid.

- **`FileDecoder`** — Decodes compressed audio files in a background thread. Uses `AMediaExtractor` + `AMediaCodec` to extract and decompress audio; converts decoder output to float32 (handles int16, int32, and float PCM formats); resamples to the stream's sample rate if needed via linear interpolation; upmixes mono to stereo; writes to an `oboe::FifoBuffer` (2 second ring buffer, lock-free). Audio thread calls `read()` to drain frames with zero-copy; `isEOF()` signals when all samples have been consumed. Methods: `open(fd, offset, length)` (also stores `mDurationFrames` from `AMEDIAFORMAT_KEY_DURATION`), `start()`, `stop()`, `pause()` (stops decode thread, flushes FIFO), `resume(frameOffset)` (seeks extractor, flushes codec, clears FIFO, restarts thread), `seekToStart()` (calls `resume(0)`), `read(...)`, `isEOF()`, `getFramesConsumed()` (atomic counter), `getDurationFrames()` (total duration in output frames). Thread model: `open`/`start`/`stop`/`pause`/`resume` on main thread; `read`/`isEOF`/`getFramesConsumed` on audio callback thread (no allocations in read path).
...
- **`jni_bridge.cpp`** — JNI glue: `nativeCreate`, `nativeDestroy`, `nativeLoadFile`, `nativeStart`, `nativeStop`, `nativePause`, `nativeResume`, `nativeSeekToStart`, `nativeIsPlaying`, `nativeSetBpm`, `nativeSetBeatsPerBar`, `nativeSetTrackVolume`, `nativeSetClickVolume`, `nativeGetDurationMs`, `nativeGetPositionMs`, `nativeSeekTo`, `nativeSetFirstBeatOffset`, `nativeAnalyzeBpm` (returns `jlongArray[2]` = `{bpm, firstBeatFrames}`). Stores native `AudioEngine` pointer in a `jlong` handle passed to/from Kotlin.
